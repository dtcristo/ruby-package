...
module Package
  ...
  def self.extract_exports(box)
    # Each box runs its own isolated copy of Package, so EXPORT and Exports
  ...
  module Kernel
    ...
    def export(*args, **kwargs)
      ...
      if value.is_a?(Hash)
        ...
        Package.inject_methods(exports_module, value)

        # Package::Exports does not leak between boxes — see extract_exports.
        Package.const_set(:Exports, exports_module)
      else
        # Package::EXPORT does not leak between boxes — see extract_exports.
        Package.const_set(:EXPORT, value)
      end
    end
  end
end
